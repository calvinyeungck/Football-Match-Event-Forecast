...
#%% get data subset for model building
...
print(weight_action_class)
weight_action_class[2]=weight_action_class[2]*dribble_weight
weight_action_class[0]=weight_action_class[0]*pass_weight
weight_action_class[1]=weight_action_class[1]*end_weight
print(weight_action_class)
...
class test_data():
    ...
    def __getitem__(self, i):
        ...
        y = torch.from_numpy(y.to_numpy(dtype="float64"))
        return x,  y
#%% and get iteration data

#iteration_x=x[i:window]
#iteration_y=x[window]
#%% model
...
test_loader = DataLoader(test_dataset,shuffle=False,batch_size=batch_size,num_workers=num_workers,drop_last=True)
...
if __name__ == '__main__': 
    ...
    train_loss,train_time,trainable_params_num,model_parameter,valid_loss,test_loss=model_train(epochs)
    ...
    train_loss.to_csv("train_loss.csv",index=False) #train loss
    valid_loss.to_csv("valid_loss.csv",index=False) #valid loss
    test_loss.to_csv("test_loss.csv",index=False) #test loss
    torch.save(model_parameter, "model.pt") #model parameter
    train_time_num_params.to_csv("train_time_num_params.csv",index=False) # training time (0)and number of trainableparameters
